你现在是我的项目工程助手。我现在要完成LoRA微调类项目,Stage1-Cleaning部分的代码目标是得到用于SFT的数据集，本部分是根据这个数据集，对0.6B模型进行微调。请你先帮我搭建一个清晰、可维护、可复现的项目结构，而不是直接把所有代码写进 main.py。
