你现在是我的项目工程助手。我现在要完成对LoRA微调完的模型进行测评的项目,Stage2-LoRA-Finetune部分的代码目标是得到LoRA微调后的模型，本部分利用测试集，对得到的模型进行评测。请你先帮我搭建一个清晰、可维护、可复现的项目结构，而不是直接把所有代码写进 main.py。
...
- models/: 放LoRA的基模型
- evaluation/: 和测评相关的结果
- lora/:放置模型lora微调的相关参数，包括adapter_config.json和adapter_model.safetensors等
